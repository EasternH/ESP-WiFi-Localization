...
void tcp_client_write_task(void *arg)
{
    ...
    while (mwifi_is_connected()) {
        ...
        for (int delta=abs((offline_data[0][0]-node1)*(offline_data[0][0]-node1))+abs((offline_data[0][1]-node2)*(offline_data[0][1]-node2))+abs((offline_data[0][2]-node3)*(offline_data[0][2]-node3)),smallest=delta,i=0;i<9;i++)
        {
        ...
        //根节点tcp向外部发送数据
        //int json_size   = asprintf(&json_data, "addr: " MACSTR ", %s",
        //                           MAC2STR(src_addr), data);
        //int json_size   = asprintf(&json_data, "%s", data);
        int json_size   = asprintf(&json_data, "Result %s %s", position[position_number], data);
